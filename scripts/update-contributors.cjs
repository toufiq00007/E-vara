...
const prsJson = execSync(
  "gh pr list --state merged --limit 100 --json author,number,title,body,additions,deletions,url",
  { encoding: "utf8" },
);
...
for (const login in authorPrs) {
  ...
  if (!user) {
    ...
    const avatarUrl = execSync(`gh api users/${login} --jq .avatar_url`, {
      encoding: "utf8",
    }).trim();
    user = {
      id: `c_${login}`,

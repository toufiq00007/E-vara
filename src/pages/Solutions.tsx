import { Shield, Building2, Briefcase, GraduationCap, Server } from "lucide-react";
...
const SolutionsPage = () => {
  useSEO({
    ...
    description: "Explore E-VARA's autonomous identity defense solutions tailored for financial, legal, and enterprise sectors.",
  ...
  const solutions = [
    {
      ...
      description: "Protect C-suite executives and portfolio managers from targeted spear-phishing and credential stuffing attacks that bypass traditional MFA."
    },
    {
      icon: <Briefcase className="h-6 w-6 text-primary" />,
      title: "Legal Practices",
      description: "Ensure attorney-client privilege is maintained by monitoring the dark web footprints of managing partners and senior counsel."
    },
    {
      icon: <Server className="h-6 w-6 text-primary" />,
      title: "Managed Service Providers (MSPs)",
      description: "White-label the E-VARA engine to offer autonomous identity protection to your entire client portfolio with zero overhead."
    },
    {
      icon: <GraduationCap className="h-6 w-6 text-primary" />,
      title: "Public Sector & Defense",
      description: "Comply with stringent sovereign data requirements while monitoring the exposure of key government personnel and contractors."
    }
  ];
    ...
    <div className="min-h-screen bg-[#050608] text-white selection:bg-primary/30 font-mono">
      ...
      <div className="container mx-auto px-6 py-24">
        <div className="max-w-3xl mb-16">
          <h1 className="text-5xl md:text-7xl font-black tracking-tighter uppercase mb-6 leading-none">Sector-Specific <br /><span className="text-primary italic">Defense</span></h1>
          <p className="text-muted-foreground font-body text-lg leading-relaxed">
            Standard perimeter defense is obsolete. E-VARA provides sector-specific, zero-knowledge identity monitoring tailored to the unique threat models of high-liability industries.
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-6 max-w-5xl">
          {solutions.map((sol, i) => (
            <div key={i} className="p-8 rounded-2xl border border-white/5 bg-white/[0.02] hover:bg-white/[0.04] transition-colors group">
              ...
              <h3 className="text-xl font-black uppercase tracking-tight mb-3">{sol.title}</h3>
              <p className="text-muted-foreground font-body text-sm leading-relaxed">{sol.description}</p>
            </div>
          ))}
        </div>

        <div className="mt-20 p-12 rounded-[32px] border border-primary/20 bg-primary/5 text-center max-w-5xl">
          <h2 className="text-2xl font-black uppercase mb-4">Require a Custom Architecture?</h2>
          <p className="text-muted-foreground font-body mb-8 max-w-xl mx-auto">
            Our Omni-Layer tier supports dedicated edge nodes, custom SIEM integrations, and on-premise data vaults.
          </p>
          <Link to="/book-demo">

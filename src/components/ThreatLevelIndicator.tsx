...
const LEVELS = [
  { name: "DEFCON 5", min: 0, max: 20, color: "hsl(var(--severity-low))", glow: "shadow-[0_0_24px_hsl(var(--severity-low)/0.35)]" },
  { name: "DEFCON 4", min: 21, max: 40, color: "hsl(var(--severity-low))", glow: "shadow-[0_0_24px_hsl(var(--severity-low)/0.35)]" },
  { name: "DEFCON 3", min: 41, max: 60, color: "hsl(var(--severity-medium))", glow: "shadow-[0_0_24px_hsl(var(--severity-medium)/0.35)]" },
  { name: "DEFCON 2", min: 61, max: 80, color: "hsl(var(--severity-high))", glow: "shadow-[0_0_24px_hsl(var(--severity-high)/0.35)]" },
  { name: "DEFCON 1", min: 81, max: 100, color: "hsl(var(--severity-high))", glow: "shadow-[0_0_24px_hsl(var(--severity-high)/0.45)]" },
];

const ThreatLevelIndicator = ({ score }: ThreatLevelIndicatorProps) => {
  const current = LEVELS.find((level) => score >= level.min && score <= level.max) ?? LEVELS[0];
          ...
          return (
            ...
            >
              ...
              <div className="mt-1 h-1 rounded-full bg-border/40">
                <div className="h-full rounded-full" style={{ width: `${Math.min(100, Math.max(20, score))}%`, backgroundColor: level.color }} />
              </div>
            </div>
          );
        })}

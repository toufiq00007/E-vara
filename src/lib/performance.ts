...
export const initPerformanceMonitoring = () => {
  ...
  const lcpObserver = new PerformanceObserver((list) => {
    ...
    trackEvent('web_vital_lcp', { value: Math.round(lcp), rating: lcp < 2500 ? 'good' : lcp < 4000 ? 'needs-improvement' : 'poor' });
    log('info', `[Performance] LCP: ${Math.round(lcp)}ms`, { rating: lcp < 2500 ? 'good' : 'poor' });
  });
  ...
  document.addEventListener('visibilitychange', () => {
    if (document.visibilityState === 'hidden') {
      trackEvent('web_vital_cls', { value: Math.round(clsValue * 1000) / 1000, rating: clsValue < 0.1 ? 'good' : clsValue < 0.25 ? 'needs-improvement' : 'poor' });
    }
  });
  ...
  const inpObserver = new PerformanceObserver((list) => {
    for (const entry of list.getEntries()) {
      ...
      trackEvent('web_vital_inp', { value: Math.round(inp), rating: inp < 200 ? 'good' : inp < 500 ? 'needs-improvement' : 'poor' });
    }
  });
  ...
  const fcpObserver = new PerformanceObserver((list) => {
    ...
    if (entries.length > 0) {
      ...
      trackEvent('web_vital_fcp', { value: Math.round(fcp), rating: fcp < 1800 ? 'good' : 'poor' });
    }
  });
  ...
  if (navEntries.length > 0) {
    ...
    trackEvent('web_vital_ttfb', { value: Math.round(ttfb), rating: ttfb < 800 ? 'good' : 'poor' });
  }

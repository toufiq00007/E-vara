...
export const runQualityGate = (): QualityReport => {
  ...
  details.push({
    ...
    message: window.location.protocol === 'https:' ? 'HTTPS active' : 'Running without HTTPS',
  });
  ...
  details.push({
    ...
    passed: !Object.keys(import.meta.env).some(k => k.includes('SERVICE_ROLE') || k.includes('ADMIN_KEY')),
    score: !Object.keys(import.meta.env).some(k => k.includes('SERVICE_ROLE')) ? 100 : 0,
  ...
  details.push({
    ...
    message: structuredData ? 'JSON-LD structured data present' : 'Missing structured data',
  });
  ...
  // ─── Calculate Scores ───
  const categories = ['security', 'reliability', 'performance', 'accessibility', 'seo', 'ux'] as const;
  const scores: Record<string, number> = {};

  for (const cat of categories) {
    const catDetails = details.filter(d => d.category === cat);
    scores[cat] = catDetails.length > 0
      ? Math.round(catDetails.reduce((sum, d) => sum + d.score, 0) / catDetails.length)
      : 0;
  }
